...
%{
  ...
  function addMathTo(asciiMath,parentId){
  	var dom = document.createElement('p');
  	dom.innerHTML = '`'+ asciiMath +'`';
  	var parent = document.getElementById(parentId);
  	parent.appendChild(dom);
  	dom.style.visibility = 'hidden';
  	MathJax.Hub.Queue(["Typeset",MathJax.Hub,dom]);
  	MathJax.Hub.Queue(function(){dom.style.visibility = 'visible'; dom=null;});
  	// dom = null;
  }
  ...
  fracPrint = function(c1){
    ...
    if( x.den.real === 1 && x.den.imag === 0 ){
      return cpxPrint(x.num);
    }else{
  ...
  printAdd = function(c1,c2){
    var res = fracAdd(c1,c2);
    var text = fracPrint(c1)+'+'+fracPrint(c2) + ' = ' + fracPrint(res);
    addMathTo(text,'math-output');
    return res;
  };
  printSub = function(c1,c2){
    var res = fracSub(c1,c2);
    var text = fracPrint(c1)+'-'+fracPrint(c2) + ' = ' + fracPrint(res);
    addMathTo(text,'math-output');
    return res;
  };
  printMul = function(c1,c2){
    var res = fracMul(c1,c2);
    var text = fracPrint(c1)+'*'+fracPrint(c2) + ' = ' + fracPrint(res);
    addMathTo(text,'math-output');
    return res;
  };
  printDiv = function(c1,c2){
    var res = fracDiv(c1,c2);
    var text = fracPrint(c1)+' div '+fracPrint(c2) + ' = ' + fracPrint(res);
    addMathTo(text,'math-output');
    return res;
  };
  printPow = function(c1,c2){
    var res = fracAdd(c1,c2);
    var text = fracPrint(c1)+'^('+fracPrint(c2) + ') = ' + fracPrint(res);
    addMathTo(text,'math-output');
    return res;
...
e
    : e 'ADD' e           {$$ = printAdd($1,$3);}
    | e 'SUB' e           {$$ = printSub($1,$3);}
    | e 'MUL' e           {$$ = printMul($1,$3);}
    | e 'DIV' e           {$$ = printDiv($1,$3);}
    | e 'POW' e           {$$ = printPow($1,$3);}
    | 'I' e               {$$ = fracI($2); }
    | e 'I'               {$$ = fracI($1); }
