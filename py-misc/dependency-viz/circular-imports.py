...
if __name__ == '__main__':
	...
	def makeDepends( moduleName, filename ):
	...
	for match in matches:
		modules[match['name']] = makeDepends( match['name'], match['path'] )

	for aModule in modules:
		for bModule in modules:
			isAInB = modules[aModule].name in modules[bModule].moduleNames
			isBInA = modules[bModule].name in modules[aModule].moduleNames
			if isAInB and isBInA:
				print 'Circular reference (%s -> %s)' % (
					modules[aModule].name,
					modules[bModule].name
				)
