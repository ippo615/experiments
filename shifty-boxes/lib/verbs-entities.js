...
GV.addModifier('size', 'asBox toBox', function (width, height) {
    this.xSize = width;
    this.xHalf = width * 0.5;
    this.ySize = height;
    this.yHalf = height * 0.5;
...
GV.addModifier('move', 'insideX', function (xMin, xMax) {
    ...
    this.cx -= distToEdgeRight + distToEdgeLeft;
    GV.aabbFromCenter(this);
});
GV.addModifier('move', 'insideY', function (yMin, yMax) {
    ...
    this.cy -= distToEdgeBottom + distToEdgeTop;
    GV.aabbFromCenter(this);
});
...
GV.addModifier('accelerate', 'towardDir', function (goalDirection, dDirection, goalSpeed) {
    var rDirection = dDirection * GV.degToRad,
        xSpeed = goalSpeed * Math.cos(rDirection),
        ySpeed = goalSpeed * Math.sin(rDirection);
    this.xSpeed += xSpeed;
    this.ySpeed += ySpeed;
    GV.computeDirectionFromSpeed(this);
...
GV.addModifier('bounce', 'insideX', function (xMin, xMax) {
    if (GV.isOutside(xMin, this.xMin, xMax) || GV.isOutside(xMin, this.xMax, xMax)) {
        this.xSpeed = -this.xSpeed;
        GV.aabbFromCenter(this);
    }
});
GV.addModifier('bounce', 'insideY', function (yMin, yMax) {
    if (GV.isOutside(yMin, this.yMin, yMax) || GV.isOutside(yMin, this.yMax, yMax)) {
...
var UTIL = verbify();
UTIL.createVerb('sort arrange order', {}, {
    descendArray: false,
    checkObjects: false
});
